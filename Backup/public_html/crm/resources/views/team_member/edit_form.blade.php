...
<form autocomplete="off" method="post" action="{{ (isset($rec->id)) ? route( 'patch_pending_team_member', $rec->id) : route('post_team_member') }}">
    {{ csrf_field()  }}
    @if(isset($rec->id))
    {{ method_field('PATCH') }}
   ...
   <div class="col-md-7">
      ...
      <div class="form-row">
         <div class="form-group col-md-6">
            ...
            <div class="select2-wrapper">
               <?php echo form_dropdown("team_id[]", $data['team_id_list'], old_set("team_id", NULL, $rec), "class='form-control form-control-sm four-boot'") ?>
            </div>
        ...
        <div class="form-group">
            ...
            <div class="select2-wrapper">
                @if($rec)
                <select class="form-control form-control-sm four-boot" name="status">
                    <option @if($rec->status==1) selected="" @endif value="1">Active</option>
                    <option @if($rec->status==0) selected="" @endif value="0">Inactive</option>
                </select>
                @else
                <select class="form-control form-control-sm four-boot" name="status">
